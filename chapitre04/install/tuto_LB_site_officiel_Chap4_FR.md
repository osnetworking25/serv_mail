```bash
# Partie 4 : Configuration de SPF et DKIM avec Postfix sur Ubuntu Server

=== Étape 1 : Créer un enregistrement SPF dans le DNS ===
# Dans votre interface de gestion DNS, créez un nouvel enregistrement TXT comme ci-dessous.

# TXT  @   v=spf1 mx ~all
# créer un enregistrement spf dans le DNS
...
# === Étape 2: Configuring SPF Policy Agent ===

sudo apt install postfix-policyd-spf-python
# Then edit the Postfix master process configuration file.

sudo nano /etc/postfix/master.cf
# Ajoutez les lignes suivantes at the end of the file, which tells Postfix to start the SPF policy daemon when it’s starting itself.

 policyd-spf  unix  -       n       n       -       0       spawn
     user=policyd-spf argv=/usr/bin/policyd-spf
	
# Enregistrez et fermez le fichier. Next, edit Postfix main configuration file.
...
# === Étape 4: Create Signing Table, Key Table and Trusted Hosts File ===

# Créez un répertoire structure for OpenDKIM
...
# Enregistrez et fermez le fichier. Then create the key table.
...
# Enregistrez et fermez le fichier. Next, create the trusted hosts file.

sudo nano /etc/opendkim/trusted.hosts

# Ajoutez les lignes suivantes to the newly created file. This tells OpenDKIM that if an email is coming from localhost or from the same domain, then OpenDKIM should only sign the email but not perform DKIM verification on the email.
...
# === Étape 5: Generate Private/Public Keypair ===

# Create a separate folder for the domain.

sudo mkdir /etc/opendkim/keys/your-domain.com

# Générez les clés avec opendkim-genkey tool.
...
# === Étape 6: Publish Your Public Key in DNS Records ===
...
# === Étape 8: Connect Postfix to OpenDKIM ===
...
# Créez un répertoire to hold the OpenDKIM socket file and allow only opendkim user and postfix group to access it.
...
# Ajoutez les lignes suivantes at the end of this file, 
...
# So OpenDKIM will be listening on the 127.0.0.1:8892 TCP/IP socket. Enregistrez et fermez le fichier. Then edit Postfix main config file.
